...
/**
 * Created by jonsa on 26/04/2018.
 */
...
public class GameModel {
    ...
    private ArrayList<Coordinates> CPU_possible_moves;      //possible orange
    ...
    protected GameModel(int size, ArrayList<Coordinates> player, ArrayList<Coordinates> cpu) {
        this.size = size;
        this.CPU_pieces = (player == null) ? new ArrayList<Coordinates>() : player;
        this.PLAYER_pieces = (cpu == null) ? new ArrayList<Coordinates>() : cpu;
        this.PLAYER_possible_moves = new ArrayList<>();
        this.CPU_pieces = new ArrayList<>();
    ...
    protected void setInitialModel(int size) {
        //first white pieces
        Coordinates first_cpu = new Coordinates(size / 2, size / 2);
        Coordinates second_cpu = new Coordinates(first_cpu.row - 1, first_cpu.col - 1);
        this.CPU_pieces.add(first_cpu);
        this.CPU_pieces.add(second_cpu);

        //first black pieces
        Coordinates first_player = new Coordinates(first_cpu.row - 1, size / 2);
        Coordinates second_player = new Coordinates(size / 2, first_cpu.col - 1);
    ...
    protected ArrayList<Coordinates> getPossibleMoves(ArrayList<Coordinates> player, String turn) {
        ...
        if (isPlayerTurn(turn))
            ...
            return this.CPU_possible_moves;
    }
    ...
    protected Coordinates checkDneighbors(Coordinates c, String turn) {
        ...
        if (this.isPlayerTurn(turn)) {
            if (this.CPU_pieces.contains(D_pos_pos))
                return D_pos_pos;
            else if (this.CPU_pieces.contains(D_pos))
                return D_pos;
            else if (this.CPU_pieces.contains(D_neg_neg))
                return D_neg_neg;
            else if (this.CPU_pieces.contains(D_neg))
                return D_neg;
            else
                return null;
        } else {
    ...
    protected Coordinates checkHneighbors(Coordinates c, String turn) {
        ...
        if (this.isPlayerTurn(turn)) {
            if (this.CPU_pieces.contains(H_pos))
                return H_pos;
            else if (this.CPU_pieces.contains(H_neg))
                return H_neg;
            else
                return null;
        } else {
    ...
    protected Coordinates checkVneighbors(Coordinates c, String turn) {
        ...
        if (this.isPlayerTurn(turn)) {
            if (this.CPU_pieces.contains(V_pos))
                return V_pos;
            else if (this.CPU_pieces.contains(V_neg))
                return V_neg;
            else
                return null;
        } else {
    ...
    protected ArrayList<Coordinates> getFlippPiecesD(Coordinates selected, String turn) {
        ...
        if (this.isPlayerTurn(turn)) {
            while (!this.PLAYER_pieces.contains(piece_u)) {
                if (this.CPU_pieces.contains(piece_u)) {
            ...
            while (!this.PLAYER_pieces.contains(piece_d)) {
                if (this.CPU_pieces.contains(piece_d))
                    flip_pieces_D.add(piece_u);
                piece_u = new Coordinates(piece_u.row - 1, piece_u.col - 1);
    ...
    protected ArrayList<Coordinates> getFlippPiecesH(Coordinates selected, String turn) {
        ...
        if (this.isPlayerTurn(turn)) {
            while (!this.PLAYER_pieces.contains(piece)) {
                if (this.CPU_pieces.contains(piece))
    ...
    protected void flipPiece(Coordinates c) {
        if (this.PLAYER_pieces.contains(c)) {
            ...
            this.CPU_pieces.add(c);
        } else {
            this.CPU_pieces.remove(c);
    ...
    public void playGame(int time_left){
        ...
        while (!GAME_OVER) {
            ...
            if (npc_moves.size()>0){
                //pick the best move
                applyPlayerChoice(getBestMove(npc_moves) ,CPU);
            }else{
    ...
    protected Coordinates getBestMove(ArrayList<Coordinates> pieces){
        ...
        for (Coordinates c: pieces) {
            int value = 0;
            value+=getFlippPiecesV(c, CPU).size();
            value+=getFlippPiecesD(c, CPU).size();
            value+=getFlippPiecesH(c, CPU).size();
    ...
    public boolean checkIfCPUPiece(Coordinates c) {
        return this.CPU_pieces.contains(c);
    }

    public boolean checkPlayerPossible(Coordinates c) {
        return this.PLAYER_possible_moves.contains(c);
    }
    public boolean checkIfCPUPossible(Coordinates c) {
    ...
    public ArrayList<Integer> getPieces(String who) {
        ...
        if (who == PC) {
            ...
            for (Coordinates c : this.CPU_pieces) {
                result.add(this.parseCoordinates(c));
            }
        } else if (who == PCp) {
            ...
            for (Coordinates c : this.CPU_possible_moves) {
                result.add(this.parseCoordinates(c));
            }
        }
        return result;
