...
import static com.example.jonsa.reversi_game.ReversiLogic.GiveUp;

public class GridFragment extends Fragment{
    protected static final String G_PLAYER_LIST = "00";
    protected static final String G_CPU_LIST = "11";
    ...
    public void onActivityCreated(Bundle state) {
        ...
        if (state != null) {
            ...
            //load stuff
            grid_size = state.getInt(SELECTED_SIZE);
            game_timer = state.getBoolean(TIMER);
            game_player = state.getString(PLAYER_NAME);
        ...
        String templateScorePlayer = game_player + " : %s";
        ...
        if (!game_timer) {
            ...
            String m_t = String.format(templateInitialTime, String.valueOf(t_limit));
            time_t.setText(m_t);
        ...
        //cpu score text
        score_cpu = (TextView) getActivity().findViewById(R.id.game_score_cpu);
        String m_n =
                String.format(templateScoreCPU, String.valueOf(game_model.getPieces(ReversiLogic.Piece.RED)));
        score_cpu.setText(m_n);

        //remaining text
        remaining = (TextView) getActivity().findViewById(R.id.game_c_left);
        String m_r =
    ...
    public void setTurnBack() {
        ...
        for (int i = game_model.turns.size() - 1; i >= 0; i--) {
            if (game_model.whos_turn.get(i) == ReversiLogic.Piece.RED) {
                if (found == true) {
    ...
    private void UpdateStats() {
        String m_p =
                String.format(game_player + " : %s", String.valueOf(game_model.getPieces(ReversiLogic.Piece.BLACK)));
        score_p.setText(m_p);
        String m_n =
                String.format(getString(R.string.GAME_score_cpu), String.valueOf(game_model.getPieces(ReversiLogic.Piece.RED)));
        score_cpu.setText(m_n);
        String m_r =
                String.format(getString(R.string.GAME_c_left), String.valueOf(game_model.getRemainingPieces()));
        remaining.setText(m_r);
        String m_t =
    ...
    public void gameHasEnded() {
        ...
        i.putExtra(G_CPU_LIST, this.game_model.getPieces(ReversiLogic.Piece.RED));
    ...
    private void saveToDB() {
        ...
        values.put(DB_WHITE, game_model.getPieces(ReversiLogic.Piece.RED));
    ...
    protected class ReversiAdapter extends BaseAdapter {
        ...
        private void setCell(Button cell, int position) {
            ...
            if (checkPosition(position, ReversiLogic.Piece.BLACK))
                ...
                //npc
            else if (checkPosition(position, ReversiLogic.Piece.RED))
                cell.setBackgroundResource(R.drawable.red_chip);
                //plays
            else if (checkPlays(position))
                cell.setBackgroundResource(R.drawable.possible_cell);
                //rest
            else
        ...
        protected boolean checkPosition(int position, ReversiLogic.Piece color) {
            ...
            if (game_model.pieces.get(parsed_p) == ReversiLogic.Piece.BLACK && color != null && color == ReversiLogic.Piece.BLACK)
                return true;
            if (game_model.pieces.get(parsed_p) == ReversiLogic.Piece.RED && color != null && color == ReversiLogic.Piece.RED)
                return true;
            return false;
        }
